 ...
 * @subpackage Subscriptions_For_TradingView/package/gateways/wps-paypal/subscription-module-compatibility
 */
	...
	class Wps_Subscriptions_Payment_Wps_Paypal_Main {
		...
		public function __construct() {
			...
			if ( $this->wps_pifw_paypal_check_plugin_setup() ) {
				...
				add_filter( 'wps_tdv_supported_payment_gateway_for_woocommerce', array( $this, 'wps_pifw_paypal_payment_gateway_for_woocommerce' ), 10, 2 );
				add_action( 'wps_tdv_other_payment_gateway_renewal', array( $this, 'wps_pifw_process_subscription_payment' ), 10, 3 );
				add_action( 'wps_tdv_subscription_cancel', array( $this, 'wps_pifw_cancel_paypal_subscription' ), 10, 2 );
		...
		public function wps_pifw_process_subscription_payment( $order, $subscription_id, $payment_method ) {
			if ( $order && is_object( $order ) ) {
				...
				if ( 'wps_paypal' === $payment_method && 'yes' === $wps_tdv_renewal_order ) {
					...
					require_once SUBSCRIPTIONS_FOR_TRADINGVIEW_DIR_PATH . 'package/gateways/wps-paypal/class-wps-paypal-requests.php';
