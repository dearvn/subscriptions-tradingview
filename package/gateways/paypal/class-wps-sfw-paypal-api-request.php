...
class Wps_Sfw_Paypal_Api_Request {
	 ...
	 *
	 * @param string $wps_tdv_api_username wps_tdv_api_username.
	 * @param string $wps_tdv_api_password wps_tdv_api_password.
	 * @param string $wps_tdv_api_signature wps_tdv_api_signature.
	 * @param string $wps_tdv_api_version wps_tdv_api_version.
	 * @param string $wps_tdv_invoice_prefix wps_tdv_invoice_prefix.
	 * @since    1.0.2
	 */
	public function __construct( $wps_tdv_api_username, $wps_tdv_api_password, $wps_tdv_api_signature, $wps_tdv_api_version, $wps_tdv_invoice_prefix ) {

		$this->payment_args['USER'] = $wps_tdv_api_username;
		$this->payment_args['PWD'] = $wps_tdv_api_password;
		$this->payment_args['SIGNATURE'] = $wps_tdv_api_signature;
		$this->payment_args['VERSION'] = $wps_tdv_api_version;

		$this->invoice_prefix = $wps_tdv_invoice_prefix;
	 ...
	 *
	 * @name wps_tdv_get_express_checkout_param.
	 * @param array $args args for payment.
	 * @since    1.0.2
	 */
	public function wps_tdv_get_express_checkout_param( $args ) {

		// translators: placeholder is blogname.
		$default_description = sprintf( _x( 'Orders with %s', 'data sent to paypal', 'subscriptions-for-tradingview' ), get_bloginfo( 'name' ) );
		...
		$this->payment_args = array_merge(
			...
			array(
				...
				'L_BILLINGAGREEMENTDESCRIPTION0' => $this->wps_tdv_billing_agreement_description( $args['billing_description'] ),
		...
		if ( isset( $args['order'] ) ) {
			...
			if ( 0 == $args['order']->get_total() ) {
				...
				$this->wps_tdv_get_order_express_checkout_params( $args['order'], $args['payment_action'] );
			}
		}
	 ...
	 *
	 * @name wps_tdv_get_order_express_checkout_params.
	 * @param object $order order.
	 * @param string $payment_action payment_action.
	 * @param string $is_reference_payment is_reference_payment.
	 * @since    1.0.2
	 */
	public function wps_tdv_get_order_express_checkout_params( $order, $payment_action, $is_reference_payment = '' ) {
		...
		if ( $order->get_total_discount() > 0 ) {
			...
			$order_items[] = array(
				'NAME' => __( 'Total Discount', 'subscriptions-for-tradingview' ),
				'QTY'  => 1,
		...
		if ( 'reference_payment' == $is_reference_payment ) {
			...
			foreach ( $order_items as $item ) {
				$this->wps_tdv_add_line_item_params( $item, $item_count++, $is_reference_payment );
			}
			...
			foreach ( $order_items as $item ) {
				$this->wps_tdv_add_line_item_params( $item, $item_count++ );
			}
			$this->payment_args = array_merge(
				$this->payment_args,
				array(
	 ...
	 *
	 * @name wps_tdv_billing_agreement_description.
	 * @since    1.0.2
	 */
	public function wps_tdv_billing_agreement_description() {
		/* Translators: placeholder is blogname. */
		$description = sprintf( _x( 'Orders with %s', 'data sent to PayPal', 'subscriptions-for-tradingview' ), get_bloginfo( 'name' ) );
	 ...
	 *
	 * @name wps_tdv_do_express_checkout_params.
	 * @param string $token token.
	 * @param object $order order.
	 * @param array  $args args.
	 * @since    1.0.2
	 */
	public function wps_tdv_do_express_checkout_params( $token, $order, $args ) {
		...
		$this->wps_tdv_get_order_express_checkout_params( $order, $args['payment_action'] );
		return $this->payment_args;
	}
	 ...
	 *
	 * @name wps_tdv_get_express_checkout_params.
	 * @param string $token token.
	 * @since    1.0.2
	 */
	public function wps_tdv_get_express_checkout_params( $token ) {
	 ...
	 *
	 * @name wps_tdv_create_billing_agreement_params.
	 * @param string $token token.
	 * @since    1.0.2
	 */
	public function wps_tdv_create_billing_agreement_params( $token ) {
	 ...
	 * Create billing parameters.
	 *
	 * @name wps_tdv_do_reference_transaction_params.
	 * @param string $reference_id reference_id.
	 * @param string $order order.
	 * @since    1.0.2
	 */
	public function wps_tdv_do_reference_transaction_params( $reference_id, $order ) {
		...
		$this->wps_tdv_get_order_express_checkout_params( $order, $wps_params['payment_action'], 'reference_payment' );
		return $this->payment_args;
	 ...
	 *
	 * @name wps_tdv_add_line_item_params.
	 * @param array  $wps_args wps_args.
	 * @param int    $item_count item_count.
	 * @param string $reference_payment reference_payment.
	 * @since    1.0.2
	 */
	public function wps_tdv_add_line_item_params( $wps_args, $item_count, $reference_payment = '' ) {
