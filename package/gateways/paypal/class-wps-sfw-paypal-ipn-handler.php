...
if ( ! class_exists( 'WPS_Sfw_PayPal_IPN_Handler' ) ) {
	...
	class WPS_Sfw_PayPal_IPN_Handler extends WC_Gateway_Paypal_IPN_Handler {
			 ...
			 *
			 * @name wps_tdv_valid_response
			 * @param string $wps_transaction_details wps_transaction_details.
			 * @since    1.0.1
			 */
			public function wps_tdv_valid_response( $wps_transaction_details ) {
				global $wpdb;
				...
				if ( isset( $order ) ) {
					...
					WC_Gateway_Paypal::log( 'TradingView - Found order #' . $order_id );

					$wps_transaction_details['payment_status'] = strtolower( $wps_transaction_details['payment_status'] );

					WC_Gateway_Paypal::log( 'TradingView - Txn Type: ' . $wps_transaction_details['txn_type'] );

					$this->wps_tdv_process_ipn_request( $order, $wps_transaction_details );

				} else {
					WC_Gateway_Paypal::log( 'TradingView - Order Not Found.' );
				}
			}
			...
			private function wps_paypal_ipn_request( $order, $wps_transaction_details ) {
				...
				WC_Gateway_Paypal::log( 'TradingView - Transaction log payment:' . wc_print_r( $wps_transaction_details, true ) );
				$wps_order_statuses = array( 'on-hold', 'pending', 'failed', 'cancelled', 'wc-wps_renewal' );
				$wps_order_info = $this->wps_tdv_get_order_info( $wps_transaction_details );
				if ( $order->get_order_key() != $wps_order_info['order_key'] ) {
					WC_Gateway_Paypal::log( 'TradingView - Order keys not matching' );
					return;
				}
				$order_id = $order->get_id();
				...
				$wps_order_transactions   = $this->wps_tdv_validate_transaction( $wps_order_transaction_id, $wps_transaction_details );

				if ( $wps_order_transactions ) {
					update_post_meta( $order_id, '_wps_paypal_transaction_ids', $order_transactions );
				} else {
					WC_Gateway_Paypal::log( 'TradingView - Transaction ID already processed' );
					return;
				}
				...
				if ( 'yes' != $wps_order_has_susbcription ) {
					WC_Gateway_Paypal::log( 'TradingView - Not a valid Subscription' );
					return;
				}
				...
				if ( empty( $wps_subscription_id ) ) {
					...
					WC_Gateway_Paypal::log( 'TradingView - IPN subscription payment error - ' . $order_id . ' haven\'t subscriptions' );
					return;

				}
				/*check for valid subscription*/
				if ( ! wps_tdv_check_valid_subscription( $wps_subscription_id ) ) {
					WC_Gateway_Paypal::log( 'TradingView - IPN subscription payment error - ' . $order_id . ' haven\'t valid subscriptions' );
					return;
				}
				...
				switch ( $wps_transaction_details['txn_type'] ) {
					case 'subscr_signup':
						...
						$this->wps_tdv_save_post_data( $order->get_id(), $args );
						$order->add_order_note( __( 'IPN subscription started', 'subscriptions-for-tradingview' ) );
						...
						$args = array(
							'wps_tdv_paypal_transaction_id'        => $wps_transaction_details['txn_id'],
							'wps_tdv_paypal_subscriber_id'         => $wps_transaction_details['subscr_id'],

						);
						$this->wps_tdv_save_post_data( $wps_subscription_id, $args );

						break;
					case 'subscr_payment':
						// show the data in log file.
						WC_Gateway_Paypal::log( 'TradingView - Transaction log for subscr_payment:' . wc_print_r( $wps_transaction_details, true ) );
						if ( 'completed' == strtolower( $wps_transaction_details['payment_status'] ) ) {
							...
							$wps_order_transactions    = $this->wps_tdv_validate_transaction( $wps_order_transactions, $wps_transaction_details );
							if ( $wps_order_transactions ) {
								update_post_meta( $wps_subscription_id, '_wps_paypal_transaction_ids', $wps_order_transactions );
							} else {
								WC_Gateway_Paypal::log( 'TradingView - Transaction ID Error' );
								return;
							}
							$wps_pending_order    = false;
							$wps_renewal_order = $subscription->wps_renewal_subscription_order;
							WC_Gateway_Paypal::log( 'TradingView - Renewal Order ID:' . $wps_renewal_order );
							...
							WC_Gateway_Paypal::log( 'TradingView - Subscription Status:' . ( $subscription->wps_subscription_status ) );
							...
							if ( 'pending' == $subscription->wps_subscription_status || ( ! $wps_pending_order && $order->has_status( $wps_order_statuses ) ) ) {
								...
								$this->wps_tdv_save_post_data( $order->get_id(), $args );
								$order->add_order_note( __( 'IPN subscription payment completed.', 'subscriptions-for-tradingview' ) );
								$order->payment_complete( $wps_transaction_details['txn_id'] );
									...
									$this->wps_tdv_save_post_data( $wps_pending_order->get_id(), $args );

									$wps_pending_order->add_order_note( __( 'IPN subscription payment completed.', 'subscriptions-for-tradingview' ) );
									$wps_pending_order->payment_complete( $wps_transaction_details['txn_id'] );

							} else {

								$wps_renewal_order = $this->wps_tdv_create_renewal_order_for_paypal( $wps_subscription_id );

								if ( ! $wps_renewal_order ) {
									WC_Gateway_Paypal::log( 'TradingView - Renewal Order Creation failed' );
									return;
								}
								...
								$this->wps_tdv_save_post_data( $wps_renewal_order->get_id(), $args );

								$wps_renewal_order->add_order_note( __( 'IPN subscription payment completed.', 'subscriptions-for-tradingview' ) );
								$wps_renewal_order->payment_complete( $wps_transaction_details['txn_id'] );

							}
								$args = array(
									'wps_tdv_paypal_transaction_id'        => $wps_transaction_details['txn_id'],
									'wps_tdv_paypal_subscriber_id'         => $wps_sub_id,

								);
								$this->wps_tdv_save_post_data( $wps_subscription_id, $args );

								WC_Gateway_Paypal::log( 'TradingView - Subscription successfull' );
						}

						break;
					case 'subscr_cancel':
						$wps_subscriber_id = $this->wps_tdv_get_paypal_susbcriber_id( $wps_subscription_id );
						if ( $wps_subscriber_id != $wps_transaction_details['subscr_id'] ) {
							...
							$order->add_order_note( __( 'WPS-IPN subscription cancelled for this order.', 'subscriptions-for-tradingview' ) );
					...
					case 'recurring_payment_suspended_due_to_max_failed_payment':
						...
						if ( $wps_subscriber_id != $wps_transaction_details['subscr_id'] ) {
							...
							if ( $wps_pending_order ) {
								...
								$wps_pending_order->add_order_note( sprintf( __( 'TradingView - IPN Failed payment for  %$ ', 'subscriptions-for-tradingview' ), $wps_subscription_id ) );
								...
								$wps_renewal_order->add_order_note( sprintf( __( 'TradingView - IPN Failed payment for  %$ ', 'subscriptions-for-tradingview' ), $wps_subscription_id ) );
							}
							$subscription->add_order_note( __( 'TradingView - IPN Failed payment', 'subscriptions-for-tradingview' ) );
							WC_Gateway_Paypal::log( 'TradingView - IPN Failed payment:' . $subscription );
			 ...
			 *
			 * @name wps_tdv_validate_transaction
			 * @param array $wps_transaction_ids wps_transaction_ids.
			 * @param array $wps_transaction_details wps_transaction_details.
			 * @since    1.0.1
			 */
			private function wps_tdv_validate_transaction( $wps_transaction_ids, $wps_transaction_details ) {
				...
				if ( isset( $wps_transaction_details['txn_id'] ) ) {
					...
					if ( empty( $wps_transaction_ids ) || ! in_array( $transaction_id, $wps_transaction_ids ) ) {
						...
						WC_Gateway_Paypal::log( 'paypal', 'TradingView - Subscription IPN Error: IPN ' . $transaction_id . ' message has already been correctly handled.' );
						return false;
					}
					...
					if ( empty( $wps_transaction_ids ) || ! in_array( $track_id, $wps_transaction_ids ) ) {
						...
						WC_Gateway_Paypal::log( 'paypal', 'TradingView - Subscription IPN Error: IPN ' . $track_id . ' message has already been correctly handled.' );
						return false;
					}
				}
			 ...
			 *
			 * @name wps_tdv_save_post_data
			 * @param int   $order_id order_id.
			 * @param array $args args.
			 * @since    1.0.1
			 */
			private function wps_tdv_save_post_data( $order_id, $args ) {
			 ...
			 *
			 * @name wps_tdv_get_order_info
			 * @param array $args args.
			 * @since    1.0.1
			 */
			private function wps_tdv_get_order_info( $args ) {
			 ...
			 *
			 * @name wps_tdv_get_paypal_susbcriber_id
			 * @param int $subscription_id subscription_id.
			 * @since    1.0.1
			 */
			private function wps_tdv_get_paypal_susbcriber_id( $subscription_id ) {
				$wps_subscriber_id = '';
				if ( isset( $subscription_id ) && ! empty( $subscription_id ) ) {
					$wps_subscriber_id = get_post_meta( $subscription_id, 'wps_tdv_paypal_subscriber_id', true );
				}
			 ...
			 *
			 * @name wps_tdv_create_renewal_order_for_paypal
			 * @param int $subscription_id subscription_id.
			 * @since    1.0.1
			 */
			private function wps_tdv_create_renewal_order_for_paypal( $subscription_id ) {
				$wps_renew_order = false;
				WC_Gateway_Paypal::log( 'TradingView - Renewal Order result' . $subscription_id );
				if ( wps_tdv_check_valid_subscription( $subscription_id ) ) {
					WC_Gateway_Paypal::log( 'TradingView - Renewal Order result1:' . $subscription_id );
					$current_time = current_time( 'timestamp' );
					$subscription = get_post( $subscription_id );
					// show the data in log file.
					WC_Gateway_Paypal::log( 'TradingView - Renewal Order subscription1:' . wc_print_r( $subscription, true ) );
					// show the data in log file.
					WC_Gateway_Paypal::log( 'TradingView - Renewal Order subscription2:' . wc_print_r( $subscription, true ) );
					$parent_order_id  = $subscription->wps_parent_order;
					WC_Gateway_Paypal::log( 'TradingView - Renewal Order result parent order:' . $parent_order_id );
					...
					WC_Gateway_Paypal::log( 'TradingView - Renewal Order result order_id:' . $order_id );
					...
					update_post_meta( $order_id, 'wps_tdv_renewal_order', 'yes' );
					update_post_meta( $order_id, 'wps_tdv_subscription', $subscription_id );
					update_post_meta( $order_id, 'wps_tdv_parent_order_id', $parent_order_id );

					do_action( 'wps_tdv_renewal_order_creation', $wps_new_order, $subscription_id );
					...
					$wps_next_payment_date = wps_tdv_next_payment_date( $subscription_id, $current_time, 0 );

					update_post_meta( $subscription_id, 'wps_next_payment_date', $wps_next_payment_date );
					do_action( 'wps_tdv_other_payment_gateway_renewal', $wps_new_order, $susbcription_id, $payment_method );
					return $wps_new_order;
				}
				WC_Gateway_Paypal::log( 'TradingView - Renewal Order result  failed:' . $subscription_id );
