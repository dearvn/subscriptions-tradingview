...
if ( ! class_exists( 'Wps_Subscriptions_Payment_Woocybs_Main' ) ) {
	...
	class Wps_Subscriptions_Payment_Woocybs_Main {
		...
		public function __construct() {
			...
			if ( $this->wps_tdv_check_woo_cybs_enable() && wps_tdv_check_plugin_enable() ) {

				add_action( 'wps_tdv_subscription_cancel', array( $this, 'wps_wsp_cancel_woo_cybs_subscription' ), 10, 2 );
				...
				add_filter( 'wps_tdv_supported_payment_gateway_for_woocommerce', array( $this, 'wps_wsp_woo_cybs_payment_gateway_for_woocommerce' ), 10, 2 );
				add_action( 'wps_tdv_other_payment_gateway_renewal', array( $this, 'wps_wsp_woo_cybs_process_subscription_payment' ), 10, 3 );
		...
		public function wps_wsp_woo_cybs_save_payment_token( $payment_token, $order_id ) {
			if ( ! empty( $payment_token ) ) {
				$wps_has_subscription = get_post_meta( $order_id, 'wps_tdv_order_has_subscription', true );
		...
		public function wps_wsp_woo_cybs_process_subscription_payment( $order, $subscription_id, $payment_method ) {
			...
			if ( $order && is_object( $order ) ) {
				...
				if ( ! $this->wps_wsp_check_supported_payment_options( $payment_method ) || 'yes' != $wps_tdv_renewal_order ) {
					return;
				}
				...
				if ( class_exists( 'CybsSoapiCC' ) ) {
					...
					if ( empty( $payment_token ) ) {
						...
						$order->update_status( 'failed', sprintf( __( 'Payment token not found %s', 'subscriptions-for-tradingview' ), $order->get_payment_method_title() ) );
						return;
					}
					...
					if ( strcmp( $decision, 'ACCEPT' ) == 0 && strcmp( $reason_code, '100' ) == 0 ) {
						...
						$order->add_order_note( sprintf( __( 'Renewal Order is successfully paid!. Cardholder: %1$s Last 4 card digits: : %2$s', 'subscriptions-for-tradingview' ), $card_name, $numero_cuenta ) );
					}
				}
			}
		}
		 ...
		 *
		 * @name wps_tdv_check_woo_cybs_enable
		 * @since 1.3.0
		 * @return boolean
		 */
		public function wps_tdv_check_woo_cybs_enable() {
			$activated = false;
