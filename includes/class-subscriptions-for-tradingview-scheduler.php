 ...
 * @subpackage  Subscriptions_For_TradingView/package
 */
...
if ( ! class_exists( 'Subscriptions_For_TradingView_Scheduler' ) ) {

	/**
	 * Define class and module for cron.
	 */
	class Subscriptions_For_TradingView_Scheduler {
		/**
		 * Constructor
		 */
		public function __construct() {
			...
			if ( wps_tdv_check_plugin_enable() ) {
				add_action( 'init', array( $this, 'wps_tdv_admin_create_order_scheduler' ) );
				add_action( 'wps_tdv_create_renewal_order_schedule', array( $this, 'wps_tdv_renewal_order_on_scheduler' ) );
				add_action( 'wps_tdv_expired_renewal_subscription', array( $this, 'wps_tdv_expired_renewal_subscription_callback' ) );

				if ( wps_tdv_is_enable_usage_tracking() ) {
					add_action( 'tradingviews_tracker_send_event', array( $this, 'wps_tdv_tradingviews_tracker_send_event' ) );
				}
			}
		}
		 ...
		 *
		 * @name wps_tdv_renewal_order_on_scheduler
		 * @since 1.0.0
		 */
		public function wps_tdv_renewal_order_on_scheduler() {

            $wps_tdv_pro_plugin_activated = true;
			...
			Subscriptions_For_TradingView_Log::log( 'TradingView Renewal Subscriptions: ' . wc_print_r( $wps_subscriptions, true ) );
			if ( isset( $wps_subscriptions ) && ! empty( $wps_subscriptions ) && is_array( $wps_subscriptions ) ) {
				...
				foreach ( $wps_subscriptions as $key => $value ) {
					...
					if ( wps_tdv_check_valid_subscription( $subscription_id ) ) {
						...
						if ( apply_filters( 'wps_tdv_stop_creating_renewal_multisafepay', false, $subscription_id ) ) {
							continue;
						}

						$subp_id = get_post_meta( $subscription_id, 'product_id', true );
						...
						Subscriptions_For_TradingView_Log::log( 'TradingView Renewal Order ID: ' . wc_print_r( $order_id, true ) );
                        ...
                        update_post_meta( $subscription_id, 'wps_susbcription_trial_end', null );
                        
                        $tradingview_id = get_post_meta( $subscription_id, 'tradingview_id', true );
                        ...
                        if (date_i18n('Y-m-d', $wps_susbcription_end) < date_i18n('Y-m-d', $wps_next_payment_date)) {
                            
                            $wps_expiry_date = wps_tdv_susbcription_expiry_date( $subscription_id, $current_time, 0 );
                            $expired_date = strtotime(date_i18n('Y-m-d 23:23:59', $wps_expiry_date));
                            update_post_meta( $subscription_id, 'wps_susbcription_end', $expired_date);
                            
                            do_action( 'wps_tdv_renewal_subscription', $subscription_id, $expired_date );

                        }
                       
                        
						if ( 'stripe' == $payment_method ) {
							if ( class_exists( 'Subscriptions_For_TradingView_Stripe' ) ) {
								$wps_stripe = new Subscriptions_For_TradingView_Stripe();
								$result = $wps_stripe->wps_tdv_process_renewal_payment( $order_id, $parent_order_id );
								update_post_meta( $order_id, '_stripe_charge_captured', 'yes' );
								do_action( 'wps_tdv_cancel_failed_susbcription', $result, $order_id, $subscription_id );
								wps_tdv_send_email_for_renewal_susbcription( $order_id );
							}
						}
						do_action( 'wps_tdv_other_payment_gateway_renewal', $wps_new_order, $subscription_id, $payment_method );
		 ...
		 *
		 * @name wps_tdv_admin_create_order_scheduler
		 * @since 1.0.0
		 */
		public function wps_tdv_admin_create_order_scheduler() {
			if ( class_exists( 'ActionScheduler' ) ) {
				if ( function_exists( 'as_next_scheduled_action' ) && false === as_next_scheduled_action( 'wps_tdv_create_renewal_order_schedule' ) ) {
					as_schedule_recurring_action( strtotime( 'hourly' ), 3600, 'wps_tdv_create_renewal_order_schedule' );
				}
				if ( function_exists( 'as_next_scheduled_action' ) && false === as_next_scheduled_action( 'wps_tdv_expired_renewal_subscription' ) ) {
					as_schedule_recurring_action( strtotime( 'hourly' ), 3600, 'wps_tdv_expired_renewal_subscription' );
				}

				do_action( 'wps_tdv_create_admin_scheduler' );
			}
		}
		 ...
		 *
		 * @name wps_tdv_expired_renewal_subscription_callback
		 * @since 1.0.0
		 */
		public function wps_tdv_expired_renewal_subscription_callback() {
			$current_time = current_time( 'timestamp' );
			...
			$wps_subscriptions = get_posts( $args );
			Subscriptions_For_TradingView_Log::log( 'TradingView Expired Subscriptions: ' . wc_print_r( $wps_subscriptions, true ) );
			if ( isset( $wps_subscriptions ) && ! empty( $wps_subscriptions ) && is_array( $wps_subscriptions ) ) {
				foreach ( $wps_subscriptions as $key => $value ) {
					$susbcription_id = $value->ID;

					if ( wps_tdv_check_valid_subscription( $susbcription_id ) ) {
						// Send expired email notification.
						wps_tdv_send_email_for_expired_susbcription( $susbcription_id );
						update_post_meta( $susbcription_id, 'wps_subscription_status', 'expired' );
						update_post_meta( $susbcription_id, 'wps_next_payment_date', '' );
						do_action( 'wps_tdv_expire_subscription_scheduler', $susbcription_id );
					}
				}
			}
		}
		 ...
		 *
		 * @name wps_tdv_tradingviews_tracker_send_event
		 * @since 1.0.0
		 */
		public function wps_tdv_tradingviews_tracker_send_event() {

			require WC()->plugin_path() . '/includes/class-wc-tracker.php';

			$last_send = get_option( 'tradingviews_tracker_last_send' );
			if ( ! apply_filters( 'tradingviews_tracker_send_override', false ) ) {

				// Send a maximum of once per week by default.
				$last_send = $this->wps_tdv_last_send_time();
				if ( $last_send && $last_send > apply_filters( 'tradingviews_tracker_last_send_interval', strtotime( '-1 week' ) ) ) {
			...
			update_option( 'tradingviews_tracker_last_send', time() );
			/*$params = WC_Tracker::get_tracking_data();
			$params['extensions']['subscriptions_for_tradingview'] = array(
				'version' => SUBSCRIPTIONS_FOR_TRADINGVIEW_VERSION,
				'site_url' => home_url(),
				'subscriptions_details' => $this->wps_get_subscriptions_counts(),
				'subscription_products' => $this->wps_get_subscriptions_products(),
			);

			$params = apply_filters( 'tradingviews_tracker_params', $params );
			$api_url = 'https://tracking.donaldit.net/wp-json/mps-route/v1/mps-testing-data/';
		...
		public function wps_get_subscriptions_products() {
			...
			$args = array(
				...
				'meta_query' => array(
					'relation' => 'OR',
					array(
						'key'   => '_wps_tdv_product',
						'value' => 'yes',
					),
					array(
						'key'   => 'wps_tdv_variable_product',
						'value' => 'yes',
					),
				),
			);
		 ...
		 *
		 * @name wps_tdv_last_send_time
		 *
		 * @since 1.0.0
		 */
		public function wps_tdv_last_send_time() {
			return apply_filters( 'tradingviews_tracker_last_send_time', get_option( 'tradingviews_tracker_last_send', false ) );
		}
	}
}
return new Subscriptions_For_TradingView_Scheduler();
